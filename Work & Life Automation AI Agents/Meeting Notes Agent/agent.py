...
from prompts import SYSTEM_PROMPT, get_meeting_prompt
...
def process_meeting(transcript, model="gpt-4o-mini", api_key=None):
    """
    Processes meeting transcripts into structured tactical intelligence.
    """
    if not transcript:
        return {"error": "Wait! A transcript or notes are required for processing."}
    ...
    # Prepare Prompt
    user_prompt = get_meeting_prompt(transcript)
