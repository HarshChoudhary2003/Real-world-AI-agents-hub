...
def build_plan(tasks: List[Task]) -> Dict:
    ...
    # Step 7 Logic: Time limit constraint
    todays_tasks = []
    current_effort = 0
    remaining_unblocked = []
    ...
    for item in unblocked:
        task = item[0]
        if current_effort + task.effort_min <= AVAILABLE_MIN:
            todays_tasks.append(item)
            current_effort += task.effort_min
        else:
            remaining_unblocked.append(item)
            
    top3 = todays_tasks[:TOP3_COUNT]
    next5 = todays_tasks[TOP3_COUNT:TOP3_COUNT + NEXT5_COUNT]
 
    # Defer: remaining unblocked that are low urgency and low impact or didn't fit in available time
    defer = []
    for (t, s, b) in remaining_unblocked:
        defer.append((t, s, b))
    ...
    plan = {
        ...
        "daily_effort_allocated": current_effort,
        "daily_effort_limit": AVAILABLE_MIN,
        "top3": serialize(top3),
...
def render_summary(plan: Dict) -> str:
    ...
    lines.append(f"Time allocated: {plan.get('daily_effort_allocated', 0)}m / {plan.get('daily_effort_limit')}m")
    lines.append("=" * 45)
    ...
    section("DEFER / BACKLOG (Low priority or no time)", plan["defer"])
